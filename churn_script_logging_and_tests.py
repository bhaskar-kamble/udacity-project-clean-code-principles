...
def test_import():
    ...
    try:
        ...
        logging.error(
            "ERROR in import_data: The file doesn't appear to have rows and columns")
        raise err
...
def test_eda(dfinput):
    ...
    try:
        ...
        eda_image_paths = ["./images/eda/" +
                           _ +".png" for _ in eda_image_paths]
        for eip in eda_image_paths:
            assert os.path.exists(eip)
    except AssertionError as err:
        logging.error(
            "ERROR in perform_eda: at least one of the figures was not saved or generated.")
        raise err
...
def test_encoder_helper(dfinput):
    ...
    try:
        ...
        logging.error(
            "ERROR in encoder_helper: The file doesn't appear to have rows and columns")
        raise err
    try:
...
def test_perform_feature_engineering(dfinput):
    ...
    try:
        ...
        logging.error(
            "ERROR in perform_feature_engineering: Data seems to be empty.")
        raise err
...
def test_train_models(train_test_data):
    ...
    try:
        ...
        result_image_paths = ["./images/results/" +
                              _ + ".png" for _ in result_image_paths]
